...
\textbf{Stage two: dual-memory diagnostics.} Table~\ref{tab:dualresults} evaluates the dual variants on exactly the failure episodes. The headline outcomes: dual-correction-stale with QwenVL \emph{fixes episode 2} (which even oracle symbolic memory fails); episode 3 resists every variant---the symbolic sequence reaches the stop stage but low-level execution fails; and on episode 12 the perceptual-verifier controller is the only configuration that behaves correctly, holding the current stage when recent frames show the pick is incomplete, where a hard timeout would advance the count prematurely.
